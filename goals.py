...
class GoalsWindow(MainCanvas):
    ...
    def build_window(self):
        ...
        self.create_text(1080 * self.res[0], 60 * self.res[1], text="Create goals for today", font=self.settings.font,
        ...
        self.create_image(75 * self.res[0], 750 * self.res[1],
                          image=create_imagetk("images/goals/arrow.png", 50, int(1140 * self.res[1])))
        self.create_text(20 * self.res[0], 750 * self.res[1], text="Importance", font=self.settings.font,
                         fill=self.settings.font_color, anchor="nw", angle=90)

        self.e_todo = CTkEntry(self.app, font=("Arial", 20))
        self.e_todo.focus()
        self.create_window(1030 * self.res[0], 1295 * self.res[1], window=self.e_todo, width=1760 * self.res[0],
                           height=50)
        ...
        self.create_window(75 * self.res[0], 1295 * self.res[1], window=b_add, height=50, width=50)
        ...
        b_submit = CTkButton(self.app, text="Submit", font=("Arial", 30), fg_color=self.settings.second_color,
                             ...
                             border_width=5, command=self.app.c_timeline.build_timeline)
        self.create_window(2035 * self.res[0], 1295 * self.res[1], window=b_submit, width=150, height=50)

        self.dots = self.create_image(125, 210, image=create_imagetk("images/goals/dots.png", 50, 50),
                                      tags=("dots",), state='hidden')
        self.shadow = self.create_text(-100, -100, text="", font=("Arial", 20), fill="grey")
        self.itemconfigure(self.shadow, state='hidden')
        self.line = self.create_image(-100, -100 + self.shadow_line_position * 60,
                                      image=create_imagetk("images/line2.png"), state='hidden')

        self.tag_bind("dots", "<B1-Motion>", self.move_dots)
        self.tag_bind("dots", "<Button-1>", self.press_dots)
        self.tag_bind("dots", "<ButtonRelease-1>", self.unpress_dots)
    ...
    def show_goal(self, text):
        ...
        goal = self.create_text(150 * self.res[0], (140 + i * 60) * self.res[1], text=f"{text}",
                                font=("Arial", 20),
        ...
        self.tag_bind(f"todo{i}", '<Enter>', self.strike_on)
        self.tag_bind(f"todo{i}", '<Leave>', self.strike_off)
        self.tag_bind(f"todo{i}", '<Button-1>', self.delete_goal)
    ...
    def delete_goal(self, event):
        ...
        for i in range(index, len(self.goals_widgets) - 1):
            next_text = self.itemcget(self.goals_widgets[i + 1], 'text')
            self.itemconfigure(self.goals_widgets[i], text=next_text)

        self.delete(self.goals_widgets[-1])
        self.goals_widgets.pop()
        self.goal_widgets_yposes.pop()

        if len(self.goals_widgets) != 0:
            self.moveto(self.dots, 100 * self.res[0], (self.goal_widgets_yposes[-1] - 25) * self.res[0])
    ...
    def strike_on(self, event):
        ...
        None
        """
        self.itemconfigure(event.widget.find_withtag("current")[0], font=("Arial", 20, "overstrike", "bold"))
    ...
    def strike_off(self, event):
        ...
        None
        """
        self.itemconfigure(event.widget.find_withtag("current")[0], font=("Arial", 20))
    ...
    def position(self, event):
        ...
        if flag == 1 and (y - 14) * self.res[0] < event.y < (y + 14) * self.res[1]:
            self.itemconfigure(self.dots, state='normal')
            self.moveto(self.dots, 100 * self.res[0], (y - 25) * self.res[0])
        else:
            self.itemconfigure(self.dots, state='hidden')
    ...
    def press_dots(self, event):
        ...
        for posy in self.goal_widgets_yposes:
            if (posy - 25) * self.res[0] < event.y < (posy + 25) * self.res[1]:
                text = self.itemcget(self.goals_widgets[self.goal], 'text')
                flag = 1
            elif flag == 0:
                self.goal += 1
        self.itemconfigure(self.shadow, text=text)
        self.unbind("<Motion>")
    ...
    def move_dots(self, event):
        ...
        self.moveto(self.line, 150 * self.res[0], (150 + self.shadow_line_position * 60) * self.res[1])

        if 200 * self.res[1] < event.y < self.goal_widgets_yposes[-1] * self.res[1]:
            self.moveto(self.dots, 100 * self.res[0], (event.y - 25))
            self.moveto(self.shadow, 150 * self.res[0], (event.y - 20))
    ...
    def unpress_dots(self, *_):
        ...
        self.bind('<Motion>', self.position)
        self.itemconfigure(self.shadow, state="hidden")
        self.moveto(self.line, -100, -100)
        self.moveto(self.shadow, -100, -100)
        ...
        for i in range(len(self.goals_widgets)):
            self.itemconfigure(self.goals_widgets[i], text=self.goals_texts[i])
